...
class BuildTabBarLayout extends ConsumerStatefulWidget {
  final List<Tab> tabs;
  final List<Widget> tabSliverPages; // <-- كل تاب: صفحة تحتوي Slivers فقط
  final Color? backgroundColorAppBar;
  ...
  const BuildTabBarLayout({
    ...
    this.expandedHeight = 250.0,
    this.initialTabIndex = 0,
  }) : assert(tabs.length == tabSliverPages.length);

  @override
  ConsumerState<BuildTabBarLayout> createState() =>
      _BuildTabBarLayoutState();
}
...
class _BuildTabBarLayoutState extends ConsumerState<BuildTabBarLayout>
  ...
  void initState() {
    ...
    _scrollControllers =
        List<ScrollController?>.filled(widget.tabs.length, null);
  }

  ScrollController _ensureScrollController(int index) {
    return _scrollControllers[index] ??= ScrollController();
  }
  ...
  void _handleTabChange() {
    ...
    final controller = _scrollControllers[newIndex];
          ...
          SliverOverlapAbsorber(
            ...
            sliver: MultiSliver(
              children: [
                ...
                /// ----------- 2) TabBar مثبت باستخدام SliverPinnedHeader -----------
                SliverPinnedHeader(
                  child: Material(
                    elevation: 2,
                    child: CustomTabBar(
      ...
      body: Padding(
        ...
        child: TabBarView(
          ...
          children: List.generate(
            widget.tabSliverPages.length,
            (index) => Builder(
              builder: (context) => CustomScrollView(
                controller: _ensureScrollController(index),
                key: PageStorageKey("tab_sliver_$index"),
                physics: const ClampingScrollPhysics(),
                slivers: [
                  SliverOverlapInjector(
                    handle: NestedScrollView.sliverOverlapAbsorberHandleFor(
